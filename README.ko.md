...
| `W20` | 뒤에 그려진 것에 글자가 묻힌 경우(대비 2.0:1 미만): 차트 막대를 가로지르는 각주, 색 패널에 떨어진 라벨, 슬라이드 배경 위에 그대로 남은 유령 텍스트, 그리고 svgBlip 벡터 차트 안에 묻힌 캐프션까지. 아래 도형이 여럿이면 덮인 면적을 합산하고, 해독 불가 채우기와 배경은 기권으로 알린다. 렌더 없이 XML 색과 좌표로 판정, 덮임 기준 소크 동안 `full` 프로필 전용 |
| `W21` | 덱 전체에 쓰이는 색 바로 옆에 거의 같은 색이 몇 번만 칠해진 경우. 색을 잘못 입력하면 밖에서 이렇게 보인다. 페이지가 아니라 덱 단위로 판정하고, redmean 거리 16 이하에 지배색 사용 횟수가 6배 이상일 때만 잡는다. 의도한 그러데이션은 양쪽 다 근접 이웃이 둘 이하여야 한다는 조건으로 걸러낸다. 램프와 오타를 가르는 것이 이 조건이다. 148덱 실측에서 95%가 0건, 최대 2건. 임계값 소크 동안 `full` 프로필 전용 |
| `W22` | 글자가 가는 궘선을 관통당하는 경우: 선의 중심선이 글리프 박스 내부를 지나고 글자 폭의 절반 이상을 덮을 때. 의도하지 않은 취소선처럼 보이는 결함이다. 밑줄과 구분선은 글리프 박스 밖이라 기하로 통과한다. 임계값 소크 동안 `full` 프로필 전용 |

프로파일이 객관 결함과 스타일 정책을 분리하고, 0.4.0부터 기본값이 `core`입니다.
기계적 게이트만(E1/E3/E4, W1/W5/W7/W8, W15~W18) 기본으로 돌고, AI 티·관행 규칙
(E2 대시, W6 반복, W9~W14)과 임계값을 소크 중인 렌더 규칙(W19~W22)은 `full` 옵트인입니다.
기계가 만든 덱을 검사하는 에이전트 루프라면 full이 맞는 모드입니다. `editorial`은
에디토리얼·포트폴리오 덱용(W6·W14와 소크 중인 W19~W22 제외).
