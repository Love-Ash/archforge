...
| `W20` | 뒤에 그려진 것에 글자가 묻힌 경우(대비 2.0:1 미만): 차트 막대를 가로지르는 각주, 색 패널에 떨어진 라벨, 그리고 슬라이드 배경 위에 그대로 남은 유령 텍스트까지. 아래 도형이 여럿이면 덮인 면적을 합산하고, 해독 불가 채우기와 배경은 기권으로 알린다. 렌더 없이 XML 색과 좌표로 판정, 덮임 기준 소크 동안 `full` 프로필 전용 |
