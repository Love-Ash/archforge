...
권민재(Ash), [@Love-Ash](https://github.com/Love-Ash). archforge가 청중보다 먼저
결함을 잡아줬다면 스타 하나가 다음 사람을 이 도구로 데려옵니다. 게이트 뒤의 실측
작업(파워포인트가 한글 폰트를 실제로 해석하는 방식, AI 덱이 조용히 깨뜨리는 것들)을
글로 정리하고 있으니 GitHub 프로필에서 이어가 주세요.
