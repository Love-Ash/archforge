...
```bash
archforge deck.pptx                 # 객관 결함만(core 프로파일, 0.4.0부터 기본)
archforge deck.pptx --profile full  # + AI 티·스타일 규칙(em dash, 레이아웃 반복 등)
...
archforge deck.pptx --write-baseline bl.json # 기존 덱을 있는 그대로 수용
...
프로젝트 기본값은 덱 폴더나 작업 폴더의 `.archforge.json`(PyYAML 설치 시 `.archforge.yml`)
에 둡니다. CLI 플래그가 설정 파일을 이깁니다.
