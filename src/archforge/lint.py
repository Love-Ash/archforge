...
ERROR = 배포 차단:
  E1 CJK 실효 렌더 폰트가 라틴 전용(한글 글리프 없음) = 조용한 Malgun 폴백.
     실효 폰트는 COM 렌더 실측 모델(2026-07-10, docs/CALIBRATION.md)로 해석한다:
     run a:ea > 테마 minorFont a:ea(비어있지 않으면 run a:latin보다 우선)
  ...
  E2 대시류 문자가 렌더 텍스트에 포함(em/en/figure dash + 수학 마이너스 U+2212·전각 하이픈 U+FF0D·박스 罫線 U+2500 등).
     기본 모드에선 정당 타이포 두 형태만 예외: 숫자 사이 en dash(2020~2024식 범위)와
     숫자 앞 U+2212(음수 부호). --strict 는 예외 없이 전부 차단.
...
LATIN_ONLY_FONTS = (
    ...
    # 산세리프(웹·디자인 덱 단골)
    "switzer", "inter", "arial", "helvetica", "calibri", "segoe ui", "roboto",
...
def dash_violations(text: str, strict: bool = False,
    ...
    """E2 위반 문자 추출. 기본 모드는 정당 타이포 두 형태를 통과시킨다:
    숫자 사이 en dash(U+2013, 연도·수치 범위)와 숫자 앞 수학 마이너스(U+2212, 음수 표기).
    em dash 등 나머지 대시류는 맥락 무관 차단. strict=True면 예외 없이 전부 차단
    (외부 리뷰 2026-07-10: 정상 범위 표기를 ERROR로 막던 오탐의 교정).

    span=(start,end)를 주면 보고는 그 구간의 문자만 하되 이웃 문맥은 text 전체에서 본다.
    호출자가 text=문단 전체, span=run 구간으로 부르면 '2020'/'–2021'처럼 run 경계로
    쪼개진 정상 범위가 오탐되지 않는다(적대 패널 확정 2026-07-10: PowerPoint는 철자검사·
    서식 경계로 run을 흔히 쪼갠다)."""
    ...
    for i in range(lo, min(hi, len(text))):
        ...
        if not strict:
            ...
            if c == _EN_DASH and prev_ch.isdigit() and next_ch.isdigit():
                continue
            if c == _MINUS and next_ch.isdigit():
                continue
        bad.append(c)
    return bad
...
def _defrpr_sz_pt(container) -> Optional[float]:
    """lvl{n}pPr 급 요소의 defRPr@sz(1/100pt 정수)를 pt로. 없거나 쓰레기값이면 None."""
    if container is None:
        return None
    d = container.find(NS + "defRPr")
    if d is None:
        return None
    sz = d.get("sz")
    if sz is None:
...
def _lst_sz_pt(lst_el, lvl: int) -> Optional[float]:
    """lstStyle 형 컨테이너(a:lvlXpPr 자식들)에서 해당 레벨의 defRPr sz(pt)."""
    if lst_el is None:
        return None
    lvl = min(max(int(lvl or 0), 0), 8)
    return _defrpr_sz_pt(lst_el.find(NS + "lvl%dpPr" % (lvl + 1)))


class SizeResolver:
    """run·문단에 명시 크기가 없을 때 OOXML 상속 체인으로 실효 크기를 해석한다.
    ...
    전부 없으면 None(=W5). 이전 버전은 run·문단만 보고 전부 W5로 흘려
    placeholder 덱에서 E3/W1/W8이 사실상 죽어 있었다(외부 리뷰 2026-07-10)."""

    def __init__(self, prs):
        self._prs = prs
        self._default_cache: Dict[int, Optional[float]] = {}
        self._default_el = None
        self._default_loaded = False
        # 상속 의존 덱은 run 수천 개가 같은 layout/master를 반복 조회한다: partname 키 메모이제이션
        # (적대 패널 지적: 기하 캐시는 넣고 정작 이 클래스는 매 run 재순회하던 비대칭)
        self._layout_cache: Dict[Tuple[str, int, int], Optional[float]] = {}
        self._master_ph_cache: Dict[Tuple[str, str, int], Optional[float]] = {}
        self._master_tx_cache: Dict[Tuple[str, str, int], Optional[float]] = {}

    def _default_text_style_sz(self, lvl: int) -> Optional[float]:
        ...
        if lvl not in self._default_cache:
            self._default_cache[lvl] = _lst_sz_pt(self._default_el, lvl)
    ...
    def _layout_ph_sz(self, slide, idx: int, lvl: int) -> Optional[float]:
        # 가드는 placeholder 단위: 루프 전체를 한 try로 감싸면 손상된 placeholder 하나가
        # 검색을 통째로 중단시켜 뒤 순번의 진짜 크기를 놓친다(적대 패널 확정 2026-07-10)
        try:
        ...
        for ph in phs:
            try:
                if ph.placeholder_format.idx == idx:
                    found = _lst_sz_pt(self._ph_lst(ph), lvl)
                    break
            except Exception:
                continue
    ...
    def _master_ph_sz(self, slide, family: str, lvl: int) -> Optional[float]:
        try:
    ...
    def _master_txstyle_sz(self, slide, family: str, lvl: int) -> Optional[float]:
        try:
            master = slide.slide_layout.slide_master
            key = (str(master.part.partname), family, lvl)
        except Exception:
        ...
        try:
            ...
            if tx is not None:
                ...
                found = _lst_sz_pt(tx.find(NS_P + tag), lvl)
        except Exception:
            found = None
    ...
    def resolve(self, tf, sp, slide, lvl: int) -> Optional[float]:
        """실효 크기(pt) 또는 None. tf=검사 중 텍스트프레임(표 셀 포함), sp=소유 도형."""
        try:
            own = _lst_sz_pt(tf._txBody.find(NS + "lstStyle"), lvl)
            if own is not None:
                return own
        except Exception:
            pass
        ...
        try:
            ...
            idx = ph_type = None
        if idx is not None:
            v = self._layout_ph_sz(slide, idx, lvl)
            if v is not None:
                return v
            family = self._ph_family(ph_type)
            v = self._master_ph_sz(slide, family, lvl)
            if v is not None:
                return v
            v = self._master_txstyle_sz(slide, family, lvl)
            if v is not None:
                return v
        return self._default_text_style_sz(lvl)
...
def lint(path, hard_min=5.0, body_min=9.0, small_min=7.5, render_dir=None, ghost=None,
    ...
    for si, slide in enumerate(prs.slides, 1):
        ...
        try:
            tboxes = _text_glyph_boxes(slide)
        except Exception as e:
            tboxes = None
            ...
            for para in paragraphs:
                ...
                for ri, run in enumerate(runs):
                    try:
                        ...
                        fonts = resolve_font_tokens(run_fonts(run), thm_fonts)

                        # E1: CJK 실효 렌더 폰트 판정(실측 렌더 모델, e1_violation 참조)
                        if has_cjk(t):
                            v = e1_violation(t, fonts, thm_ea)
                            if v is not None:
                        ...
                        if run.font.size is not None:
                            ...
                            try:
                                base_pt = sizer.resolve(tf, owner_sp, slide, getattr(para, "level", 0))
                            except Exception:
                                base_pt = None
...
def main():
    ...
    ap.add_argument("--skip", default="", metavar="CODES",
                    help="억제할 코드 콤마 목록(예 --skip W14,W6: 에디토리얼·템플릿 덱의 장르 무관 경고 제거)")
    ...
    if skip:
        errors = [e for e in errors if e[1] not in skip]
