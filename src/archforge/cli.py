# -*- coding: utf-8 -*-
"""Command-line layer: argument parsing, the subcommands, and everything that turns a
lint result into an exit code, a report file, or a line of stdout.

Extracted from lint.py for the 0.8 decomposition (#5). The dependency runs one way: this
module imports the engine, and the engine knows nothing about it. lint.py forwards these
names through a module __getattr__ so `archforge.lint:main` and the older import paths
keep working.
