...
패밀리에 따라 majorFont(제목)/minorFont(그 외) ea를 쓴다. 기본 템플릿의
...
- E1·E4는 한글 포함 런에만: 이 도구의 폰트 지식(블록리스트)이 한글 커버리지 기준이라,
  가나·한자 전용 런을 같은 지식으로 판정하면 일본어·중국어 덱에서 오탐이 납니다(2차
  재점검 실측: Noto Sans JP 오차단). 일본어 가나의 양수 자간은 정상 디자인 관행이라
  E4 대상도 아닙니다. 다른 스크립트의 심층 지원은 스크립트별 커버리지 표를 추가하는
  방식으로 확장합니다.
