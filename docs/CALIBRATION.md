...
## 알려진 한계(공개 명세, 4차 리뷰 반영)

- a:fld(슬라이드 번호·날짜 등 필드)와 a:br 줄바꿈의 텍스트는 python-pptx run 순회에
  잡히지 않아 전 게이트가 보지 못합니다. 필드는 기계 삽입 텍스트라 테마 폰트를 탈
  확률이 높은 지점이므로 0.5.0 대상입니다.
- finding location의 bbox는 도형의 raw 좌표(EMU 기반 인치)입니다. 그룹 내부 도형은
  그룹 변환 적용 전 좌표라 화면상 절대 위치와 다를 수 있습니다(기하 엔진 자체는
  변환을 적용하지만 location 페이로드는 아직 아님). 표 셀 finding의 location에
  행·열 인덱스도 아직 없습니다.
- W15~W17 기하 finding에는 location이 없습니다(관련 두 도형을 특정하는
  related_locations 구조가 0.5.0 대상).
