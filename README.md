...
이름은 arch(구조·아키텍처)와 forge(불려 벼리다·대장간)에서 왔습니다. 덱의 구조와 한글
타이포를 배포 전에 벼려 다듬는 대장간이라는 뜻입니다.
...
```bash
pip install archforge        # (배포 전이면) pip install -e <repo>

archforge deck.pptx                 # 사람용 리포트, ERROR 있으면 exit 1
archforge deck.pptx --json          # 기계 판독용(에이전트·CI)
archforge deck.pptx --strict        # WARN도 실패로
archforge deck.pptx --ghost         # 페이지별 타이틀만 나열(수평 논리 검토)
archforge deck.pptx --render pages/ # p01.png·p02.png 형식 렌더가 있으면 이미지 위 대비(W7)까지
```
...
```
빌드 → archforge --json → error_count 0 될 때까지 수정 루프 → WARN은 렌더 보고 판단
```
