...
이름은 아로새기다(곱게 새겨 넣다)의 아로에서 왔습니다. 조용히 깨지는 한글 타이포를
한 획까지 곱게 짚어 새긴다는 뜻입니다.
...
aro는 pptx 내부(XML·폰트 슬롯·좌표·이미지 알파)를 직접 읽어 빌드 직후에 잡습니다.
...
```bash
pip install aro        # (배포 전이면) pip install -e <repo>

aro deck.pptx                 # 사람용 리포트, ERROR 있으면 exit 1
aro deck.pptx --json          # 기계 판독용(에이전트·CI)
aro deck.pptx --strict        # WARN도 실패로
aro deck.pptx --ghost         # 페이지별 타이틀만 나열(수평 논리 검토)
aro deck.pptx --render pages/ # p01.png·p02.png 형식 렌더가 있으면 이미지 위 대비(W7)까지
```
...
```
빌드 → aro --json → error_count 0 될 때까지 수정 루프 → WARN은 렌더 보고 판단
```
...
aro is a CLI quality linter for built `.pptx` files with first-class Korean
